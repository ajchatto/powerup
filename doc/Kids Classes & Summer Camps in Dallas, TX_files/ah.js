...
function leadgen_link(provider_id, provider_link, web_link, reg_link){
  //perform some ajax stuff
  //need to send the csrf token for the ajax post to send the session cookie
  ...
  jQuery.ajax({
      type: "POST",
      url: "/leadgens",
      data: {'leadgen[provider_id]': provider_id, 'leadgen[web_link]': web_link, 'leadgen[registration_link]': reg_link},
      success: function (data) {
        if (data["response"] == "success") {
          window.open(provider_link);
        } 
        else
...
function toggle_state_and_zip() {
  //var v = $('#provider_location_outside_us').val();
  //alert(v);
 $('#domestic_state_and_zip').toggle();  
 $('#international_state_and_zip').toggle();  
}

function toggle_day(input_id, dom_id, klass ) {
  var v = $('#' + input_id ).val(); 
  $('#' + input_id ).val( (v == 0)  ? 1 : 0 ); 
  if ( $('#' + input_id ).val() == 0 ) {
    $('#' + dom_id ).removeClass(klass);
  } else { 
    $('#' + dom_id ).addClass(klass);
  }
}

function toggle_activity_attribute(input_id, dom_id, klass ) {
  var v = $('#' + input_id ).val(); 
  $('#' + input_id ).val( (v == 0)  ? 1 : 0 ); 
  if ( $('#' + input_id ).val() == 0 ) {
    $('#' + dom_id ).removeClass(klass);
...
function toggle_time_fields() {
  var checked = $('#activity_schedule_flexible_schedule').is(':checked');
  if ( checked ) {
    $('#activity_schedule_start_time_string').val('');
    $('#activity_schedule_end_time_string').val('');
    $('#activity_schedule_start_time_string').attr('disabled','disabled');
    $('#activity_schedule_end_time_string').attr('disabled', 'disabled');
    $('#activity_schedule_session_start_date').attr('disabled','disabled');
    $('#activity_schedule_session_end_date').attr('disabled','disabled');
    $('#refinesearch-days').hide();
    $('#meets_on_label').hide();
  } else {
    $('#activity_schedule_start_time_string').val('');
    $('#activity_schedule_end_time_string').val('');
    $('#activity_schedule_start_time_string').removeAttr('disabled');
    $('#activity_schedule_end_time_string').removeAttr('disabled');
    $('#activity_schedule_session_start_date').removeAttr('disabled');
    $('#activity_schedule_session_end_date').removeAttr('disabled');
    $('#refinesearch-days').show();
    $('#meets_on_label').show();
  }  
}

function toggle_date_fields() {
  var checked = $('#activity_schedule_flexible_start_date').is(':checked');
    $('#activity_schedule_session_start_date').attr('disabled','disabled');
    $('#activity_schedule_session_end_date').attr('disabled','disabled');
  if ( checked ) {
  } else {
...
$(document).ready(function(){
  ...
  $('#activity_category').change(function() {
     var activity_id = $("#activity_category").attr("data-id");
     var category_id = this.value;
     jQuery.ajax({
      type: "POST",
      url: '/add/categories/' + activity_id + '/' +  category_id + '/activities.js'
     });
  });
...
$(document).ready(function(){
  ...
  $('#activity_filter').change(function() {
     $('#indicator').show();
     var values = {};
      $.each($('#activity_filter').serializeArray(), function(i, field) {
        values[field.name] = field.value;
     });
     jQuery.ajax({
      type: "GET",
      url: '/search/provider/activities/' + values['provider_id'] + '.js?activity_id=' + values['activity_id'] + "&provider_location_id=" + values['provider_location_id'] 
     });
  });
...
$(document).ready(function(){
  ...
  $('#change_provider_location_form').live('change', function() {
     $('#indicator').show();
     var values = {};
     $.each($('#change_provider_location_form').serializeArray(), function(i, field) {
        values[field.name] = field.value;
     });
     jQuery.ajax({
      type: "POST",
      url: '/change/current/provider/location.js' ,
      data: {'provider_id': values['provider_id'], 'provider_location_id' : values['provider_location_id'] }
     });
  });
});
...
$(document).ready(function(){
  ...
  $('#filter_activities').live('change', function() {
     $('#indicator').show();
     var values = {};
     $.each($('#filter_activities').serializeArray(), function(i, field) {
        values[field.name] = field.value;
     });
     jQuery.ajax({
      type: "GET",
      url: '/activities/filter.js' ,
      data: {'year': values['year'], 'provider_location_id' : values['provider_location_id'] }
     });
  });
});
...
$(document).ready(function(){
  ...
  $('#provider').change(function() {
     ...
     jQuery.ajax({
      type: "GET",
      url: '/redirect/to/provider/' + provider_id + '.js'
     });
  });
...
$(document).ready(function(){
  $('#provider_short_description').simplyCountable({
    counter:            '#counter',
...
function update_provider_stats(q) {
  jQuery.ajaxSetup({
    beforeSend: function(xhr) {
      xhr.setRequestHeader('X-CSRF-Token', jQuery('meta[name="csrf-token"]').attr('content'));
    }
  ...
  jQuery.ajax({
    type: "POST"               ,
    url: '/provider_stats.js' ,
    data: {'q' : q}
  });

}

function open_folder_tab(tab) {
  $.each( ['#folder_tab_1', '#folder_tab_2', '#folder_tab_3', '#folder_tab_4', '#folder_tab_5', '#folder_tab_6', '#folder_tab_registration']  , 
	  function (i,x) { $(x).removeClass('active');} );
  $(tab).addClass('active');
}

function open_folder_tabs(outer, inner_folder, inner) {
  $.each( ['#folder_tab_1', '#folder_tab_2', '#folder_tab_3', '#folder_tab_4', '#folder_tab_5', '#folder_tab_6', '#folder_tab_registration']  , 
	  function (i,x) { $(x).removeClass('active');} );
  $(outer).addClass('active');

  $.each( ['#folder_tab41', '#folder_tab42', '#folder_tab43' ] , function (i,x) { $(x).removeClass('active'); } ); 
       $(inner_folder).addClass('active');
  $.each( ['#tab41', '#tab42', '#tab43' ] , function (i,x) { $(x).removeClass('active'); } ); 
       $(inner).addClass('active');
}

function activate_activity_photos_tab() {
  open_folder_tabs('#folder_tab_5', '#tab43' );
  $('#tab41').removeClass('active');
  $('#tab42').removeClass('active');
  $('#tab43').addClass('active');
  $('#tab43 a').tab('show');
} 

function toggleDayPicker(day_picker, day) {
  day_picker_element = document.getElementById(day_picker);
  day_element = document.getElementById(day);
  if (day_picker_element.getAttribute("class") == "selected-day") {
      day_element.value = "false"
      day_picker_element.setAttribute("class", "")
  }
  else {
      day_element.value = "true"
      day_picker_element.setAttribute("class", "selected-day")
  }
}
